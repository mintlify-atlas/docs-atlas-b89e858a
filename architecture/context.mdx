...
`FavoriteProvider` is applied at the root in `src/main.jsx`, wrapping `RouterProvider` so that context is available to every page and component in the router.

<Note>
  `FavoriteProvider` must wrap `RouterProvider` — not the other way around. Placing the provider inside the router would cause it to unmount and remount on navigation, resetting favorites state.
</Note>
